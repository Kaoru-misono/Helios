...
namespace Helios
{
    ...
    public:
        ...
        virtual auto load_texture(Texture::Kind kind, std::vector<std::string> const& paths, Texture::Format format = Texture::Format::rgb8, bool should_flip = false) -> std::shared_ptr<Texture> = 0;
        virtual auto create_texture(Texture::Kind kind, Texture::Format format, int width, int height) -> std::shared_ptr<Texture> = 0;
        virtual auto create_framebuffer() -> std::shared_ptr<RHI_Framebuffer> = 0;
        //virtual auto
    private:
    };
