...
namespace Helios
{
    ...
    auto OpenGL_RHI::load_texture(Texture::Kind kind, std::vector<std::string> const& paths, Texture::Format format, bool should_flip) -> std::shared_ptr<Texture>
    {
    ...
    auto OpenGL_RHI::create_texture(Texture::Kind kind, Texture::Format format, int width, int height) -> std::shared_ptr<Texture>
    {
        std::shared_ptr<Texture> texture;
        texture = OpenGL_Texture::create_texture(kind, format, width, height);
        return texture;
    }
